...
풀이 안내 화면에서 54자 상태 문자열을 입력하고 `풀이 생성`을 누릅니다. 결과는 `R`, `U`, `F2`, `R'` 같은 표준 표기와 한국어 설명으로 한 단계씩 표시됩니다. 시작 기준에는 위/앞/오른쪽 센터 색상이 표시되므로 실제 큐브를 같은 방향으로 잡고 첫 수를 시작합니다. 옆의 3D 큐브는 현재 인식된 스티커 색상에서 시작하며 `다음` 또는 `이전`을 누르면 해당 회전이 애니메이션으로 반영됩니다. 3D 패널은 자동으로 회전하지 않고, 별도 공식 대신 현재까지 몇 수가 반영됐는지 표시합니다. 다른 면을 확인해야 할 때는 3D 큐브를 드래그하거나 시점 제어 버튼을 사용합니다. 솔버가 물리적으로 해석할 수 없는 상태는 원문 예외 대신 재스캔/수정 안내로 표시됩니다.
