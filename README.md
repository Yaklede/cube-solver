...
풀이 안내 화면에서 54자 상태 문자열을 입력하고 `풀이 생성`을 누릅니다. 결과는 `R`, `U`, `F2`, `R'` 같은 표준 표기와 한국어 설명으로 한 단계씩 표시됩니다. 시작 기준에는 위/앞/오른쪽 센터 색상이 표시되므로 실제 큐브를 같은 방향으로 잡고 첫 수를 시작합니다. 옆의 3D 큐브는 현재 인식된 스티커 색상에서 시작하며 `다음` 또는 `이전`을 누르면 해당 회전이 애니메이션으로 반영됩니다.
