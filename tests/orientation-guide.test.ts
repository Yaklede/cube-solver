...
describe("solver orientation guide", () => {
  it("uses a white-bottom beginner grip by default", () => {
    ...
    expect(guide.map((item) => `${item.label}:${item.colorName}`)).toEqual(["아래:흰색", "앞:초록", "오른쪽:빨강"]);
    expect(formatOrientationInstruction(guide)).toBe("흰색 센터를 아래, 초록 센터를 앞, 빨강 센터를 오른쪽으로 잡고 첫 수를 시작하세요.");
  });
