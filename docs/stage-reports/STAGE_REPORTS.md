...
단계 번호: 29
단계 이름: 간단 촬영 흐름과 빈 흰 화면 오탐 차단
이번 단계 목표: 사용자가 큐브를 더 단순하게 한 면씩 촬영하고, 흰 배경/빈 화면이 흰색 면으로 자동 확정되는 문제를 막는다.
완료한 작업: 가이드 이미지에서 3x3 큐브 격자선 대비와 어두운 separator 비율을 계산하는 프레임 품질 게이트를 추가했다. 큐브 격자 신호가 없으면 자동 인식, fallback, 수동 촬영 적용을 모두 차단한다. 스캐너 화면에 `현재 면 촬영`과 `저장 후 다음 면` 중심의 간단 촬영 카드를 추가하고, 보정/진단 기능은 접힌 고급 영역으로 이동했다. 6면이 모두 저장되기 전에는 풀이 안내 이동을 비활성화한다.
생성/수정한 파일: `src/core/scan-frame.ts`, `src/features/scanner/ScannerWorkspace.tsx`, `src/app/styles.css`, `tests/scan-frame.test.ts`, `TASK.md`
핵심 구현 내용: `analyzeGuideFrameQuality`가 흰색 고신뢰도 분류와 별개로 실제 큐브 격자 존재 여부를 판단한다. `analyzeFaceReadiness`는 프레임 품질이 실패하면 `ready=false`로 내려 빈 화면을 저장하지 않는다. UI는 진행률, 현재 촬영 면, 촬영/저장 버튼을 상단에 모아 스크롤 의존도를 줄였다.
실행 방법: `npm run dev` 후 카메라/스캔 화면에서 가이드 안에 큐브 한 면을 맞추고 `현재 면 촬영`, `저장 후 다음 면` 순서로 진행한다.
테스트 방법: `npm run test -- tests/scan-frame.test.ts`, `npm run build`, `npm run validate:all`
테스트 결과: 흰 배경 오탐 차단/격자 있는 흰 면 허용 테스트 포함 대상 테스트 10개 통과, production build 통과.
스크린샷 또는 확인 가능한 결과: 스캐너 화면에 간단 촬영 카드, 6면 진행률, 비활성화된 `풀이 안내로 보내기`가 표시된다.
남은 문제: 실제 큐브 재질이 검은 테두리 없는 stickerless 타입일 때 격자 대비 기준이 과하게 엄격할 수 있어 실기기 튜닝이 필요하다.
다음 단계에서 할 일: 전체 내부검증, PR 외부검증, merge.
